                                    ...
                                    <td>
                                        @if($order->return_status !== null)
                                            @if($order->return_status == 'requested')
                                                <span class="badge bg-warning text-dark d-inline-flex align-items-center"><i
                                                        class="bi bi-hourglass-split me-1"></i>Menunggu Persetujuan Return</span>
                                            @elseif($order->return_status == 'approved')
                                                <span class="badge bg-success d-inline-flex align-items-center"><i
                                                        class="bi bi-check-circle me-1"></i>Return Disetujui</span>
                                            @else
                                                <span class="badge bg-danger d-inline-flex align-items-center"><i
                                                        class="bi bi-x-circle me-1"></i>Return Ditolak</span>
                                            @endif
                                        @else
                                            ...
                                            @elseif($order->status == 'processing')
                                                <span class="badge bg-primary d-inline-flex align-items-center"><i
                                                        class="bi bi-box me-1"></i>Processing</span>
                                            @elseif($order->status == 'delivery')
                                                <span class="badge bg-info d-inline-flex align-items-center"><i
                                                        class="bi bi-truck me-1"></i>Delivery</span>
                                            @elseif($order->status == 'completed')
                                                <span class="badge bg-success d-inline-flex align-items-center"><i
                                                        class="bi bi-check-circle me-1"></i>Completed</span>
                                            @elseif($order->status == 'cancelled')
                                                <span class="badge bg-danger d-inline-flex align-items-center"><i
                                                        class="bi bi-x-circle me-1"></i>Cancelled by System/Kurir</span>
                                            @elseif($order->status == 'returned')
                                                <span class="badge bg-secondary d-inline-flex align-items-center"><i
                                                        class="bi bi-arrow-return-left me-1"></i>Returned (Kurir)</span>
                                            @elseif($order->status == 'disposed')
                                                <span class="badge bg-dark d-inline-flex align-items-center"><i
                                                        class="bi bi-trash me-1"></i>Disposed (Paket Rusak)</span>
                                            @elseif($order->status == 'failed')
                                                <span class="badge bg-danger d-inline-flex align-items-center"><i
                                                        class="bi bi-slash-circle me-1"></i>Payment Failed</span>
                                            @else
                                                <span class="badge bg-light text-dark border d-inline-flex align-items-center"><i
        ...
        <div id="main">
            ...
            <div class="border-bottom mb-4">
                <ul class="nav nav-tabs border-0">
                    <li class="nav-item">
                        <a href="{{ route('index-admin-order') }}"
                            class="nav-link {{ request()->get('status') === null ? 'active text-primary fw-bold' : 'text-secondary' }}">
                            Semua
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('index-admin-order-need-sent', ['status' => 'pending']) }}"
                            class="nav-link {{ request()->get('status') === 'pending' ? 'active text-primary fw-bold' : 'text-secondary' }}">
                            Perlu Dikirim
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('index-admin-order-sent', ['status' => 'shipping']) }}"
                            class="nav-link {{ request()->get('status') === 'shipping' ? 'active text-primary fw-bold' : 'text-secondary' }}">
                            Dikirim
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('index-admin-order-complete-sent', ['status' => 'completed']) }}"
                            class="nav-link {{ request()->get('status') === 'completed' ? 'active text-primary fw-bold' : 'text-secondary' }}">
                            Selesai
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('index-admin-order-returned', ['status' => 'returned']) }}"
                            class="nav-link {{ request()->get('status') === 'returned' ? 'active text-primary fw-bold' : 'text-secondary' }}">
                            Pengembalian/Pembatalan/Paket Rusak
                        </a>
                    </li>
                </ul>
            ...
            <div class="alert alert-info alert-dismissible fade show mt-3" role="alert">
                <i class="fa fa-info-circle me-2"></i>
                Halaman ini menampilkan semua pesanan yang ada. Anda dapat melihat informasi pesanan lengkap dan
                statusnya di sini. Silakan perbarui atau atur pesanan sesuai kebutuhan.
                <button type="button" class="btn btn-sm btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @if ($order->return_status !== null)
                                            <div class="alert alert-secondary mt-3 p-3 mb-0"
                                                style="border: 1px dashed #6b7280; background: #f9fafb;">
                                                <div class="d-flex justify-content-between align-items-start">
                                                    <div>
                                                        <h6 class="fw-bold mb-1 text-danger"><i
                                                                class="bi bi-exclamation-triangle me-1"></i> Request
                                                            Pengembalian Manual</h6>
                                                        <p class="mb-1 text-dark fs-7"><strong>Alasan:</strong>
                                                            {{ $order->return_reason }}</p>
                                                    </div>
                                                    @if($order->return_image)
                                                        <a href="{{ Storage::url($order->return_image) }}" target="_blank"
                                                            class="btn btn-sm btn-outline-dark bg-dark">
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-2">
                                            <a href="/order-detail/{{ $order->id }}"
                                                class="badge bg-primary d-inline-flex align-items-center justify-content-center p-2 text-decoration-none">
                                                <i class="bi bi-box-arrow-in-right me-1"></i> Info Detail
                                            </a>

                                            @if($order->return_status == 'requested')
                                                <button type="button"
                                                    class="badge bg-success border-0 w-100 d-inline-flex align-items-center justify-content-center p-2"
                                                    onclick="confirmReturnAction('approve', {{ $order->id }})">
                                                    <i class="bi bi-check-lg me-1"></i> Setujui (Restore Stok)
                                                </button>
                                                <button type="button"
                                                    class="badge bg-danger border-0 w-100 d-inline-flex align-items-center justify-content-center p-2"
                                                    onclick="confirmReturnAction('reject', {{ $order->id }})">
...
<body>
                            ...
                            @foreach ($orders as $order)
                                <tr class="{{ $order->return_status == 'requested' ? 'bg-light-warning' : '' }}">
                                    ...
                                    <td>
                                        ...
                                        @else
                                            @if ($order->status == 'pending')
                                                <span class="badge bg-warning text-dark d-inline-flex align-items-center"><i class="bi bi-hourglass-split me-1"></i>Pending</span>
                                            @elseif($order->status == 'processing')
                                                <span class="badge bg-primary d-inline-flex align-items-center"><i class="bi bi-box me-1"></i>Processing</span>
                                            @elseif($order->status == 'delivery')
                                                <span class="badge bg-info d-inline-flex align-items-center"><i class="bi bi-truck me-1"></i>Delivery</span>
                                            @elseif($order->status == 'completed')
                                                <span class="badge bg-success d-inline-flex align-items-center"><i class="bi bi-check-circle me-1"></i>Completed</span>
                                            @elseif($order->status == 'cancelled')
                                                <span class="badge bg-danger d-inline-flex align-items-center"><i class="bi bi-x-circle me-1"></i>Cancelled by System/Kurir</span>
                                            @elseif($order->status == 'returned')
                                                <span class="badge bg-secondary d-inline-flex align-items-center"><i class="bi bi-arrow-return-left me-1"></i>Returned (Kurir)</span>
                                            @elseif($order->status == 'disposed')
                                                <span class="badge bg-dark d-inline-flex align-items-center"><i class="bi bi-trash me-1"></i>Disposed (Paket Rusak)</span>
                                            @elseif($order->status == 'failed')
                                                <span class="badge bg-danger d-inline-flex align-items-center"><i class="bi bi-slash-circle me-1"></i>Payment Failed</span>
                                            @else
                                                <span class="badge bg-light text-dark border d-inline-flex align-items-center"><i class="bi bi-question-circle me-1"></i>Unknown</span>
                                            @endif
                                        @endif
                                    </td>
                                    <td>
    ...
    <script>
        // Simple Datatable - MATIKAN PENGURUTAN AGAR HASIL QUERY DATABASE TIDAK BERUBAH
        let table1 = document.querySelector('#table1');
        let dataTable = new simpleDatatables.DataTable(table1, {
            sortable: false // KUNCI UTAMA
        });
        ...
        function confirmReturnAction(action, orderId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Memproses...',
                        text: 'Mohon tunggu, sedang memproses pengajuan.',
