  ...
  <div class="row gap-2 gap-md-0 m-0 p-0 mb-2">
    <div class="col-md-8 col-12 border-right border-bottom mt-3 mt-md-0">
      <div class="pb-2 mb-2 border-bottom">
        <div class="d-flex gap-1 mb-1 mb-md-2">
          <h3 class="text-[9px] md:text-[12px] lg:text-[16px] xl:text-[18px]">{{ $article->categoryArticle->name }}</h3>
          <h3 class="text-[9px] md:text-[12px] lg:text-[16px] xl:text-[18px]">|</h3>
          <h3 class="text-[9px] md:text-[12px] lg:text-[16px] xl:text-[18px]">{{ \Carbon\Carbon::parse($article->created_at)->translatedFormat('d F Y') }}</h3>
        </div>
        <div class="mb-1 mb-md-2">
          <h1 class="text-[10px] md:text-[14px] lg:text-[16px] xl:text-[18px] font-bold">{{ $article->title }}</h1>
        </div>
        <div class="d-flex">
          <h6 class="text-[9px] md:text-[12px] lg:text-[14px] xl:text-[16px]">Admin Glamoire</h6>
        </div>
      </div>
      ...
      <article class="blog-post">
        ...
        <div class="text-justify text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px] py-2">
        {!! $article->content !!}
        </div>
              ...
              <div class="row gap-2">
                ...
                @foreach ($articles as $article)
                  <div class="border-bottom pb-3 d-flex gap-2">
                    ...
                    <div>
                      <div class="d-flex gap-2 mb-1 mb-md-2">
                        <h3 class="text-[8px] md:text-[10px] lg:text-[10px] xl:text-[12px]">{{ $article->categoryArticle->name }}</h3>
                        <h3 class="text-[8px] md:text-[10px] lg:text-[10px] xl:text-[12px]">|</h3>
                        <h3 class="text-[8px] md:text-[10px] lg:text-[10px] xl:text-[12px]">{{ \Carbon\Carbon::parse($article->created_at)->translatedFormat('d F Y') }}</h3>
                      </div>
                      <div class="mb-1 mb-md-2">
                        <a href="/{{ $article->title }}_detailnewsletter">
                          <h1 class="font-bold text-[9px] md:text-[10px] lg:text-[10px] xl:text-[12px]">{{ Str::limit($article->title, 70) }}</h1>
                        </a>
                      </div>
                      <div class="d-flex">
                        <p class="text-[8px] md:text-[6px] lg:text-[8px] xl:text-[10px]">Admin Glamoire</p>
                      </div>
                    </div>
                  </div>
                @endforeach
