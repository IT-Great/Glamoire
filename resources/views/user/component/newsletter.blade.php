            ...
            <div class="row">
              ...
              @if (count($articles) !== 0)
                ...
                <div class="col-md-3 p-1 p-md-2 gap-1 col-6">
                  <div class="border border-[#183018] rounded-sm" style="min-height:5vw;">
                    <a href="/{{ $article->title }}_detailnewsletter">
                      <div class="bg-white rounded-lg shadow-smproduct-item mb-2 md:mb-4 lg:mb-4 xl:mb-4 border-xl">
                        <div class="position-relative bg-transparent p-0">
                          <img class="img-fluid w-full rounded-sm pb-1 md:pb-2 lg:pb-2 xl:pb-2" src="{{ Storage::url($article->image) }}" alt="{{$article->title}}">
                        </div>
                        <div class="text-left p-2 md:p-2 lg:p-2 xl:p-2 md:pt-4 lg:pt-4 md:pb-3 lg:pb-3">
                          <p class="text-[6px] md:text-[12px] lg:text-[12px] xl:text-[14px]">{{ $article->categoryArticle->name }} | {{ \Carbon\Carbon::parse($article->created_at)->translatedFormat('d F Y') }}</p>
