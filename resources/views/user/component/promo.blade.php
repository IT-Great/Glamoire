  ...
  <div class="container-fluid p-0 grid gap-2" style="min-height:55vh;">
    @if (count($promos) !== 0)
      <div class="col px-0 px-md-3">
        <p class="font-semibold text-[10px] md:text-[12px] lg:text-[14px] xl:text-[16px] bg-[#183018] text-white w-fit py-1 pl-1 pr-3" style="border-top-right-radius: 50px; border-bottom-right-radius: 50px;">
          Event Glamoire
        </p>
      </div>
      ...
      <div class="grid-container-promo px-3">
        @foreach ($voucherGlamoire as $voucher)
          <div class="bg-white h-fit">
            <a class="hover:cursor-pointer" data-bs-toggle="modal" data-bs-target="#voucher-{{$voucher->name}}">
              <div class="product-image-container">
                  <img class="card-img-top product-image transform transition-transform duration-300 hover:scale-110" src="{{ Storage::url($voucher->image) }}" alt="{{ $voucher->promo_name }}" title="{{$voucher->promo_name}}">
              </div>
              <div class=" p-2">
                <div class="grid w-full gap-1">
                  <div class="grid">
                    <p class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px] text-[#183018] font-semibold text-center">{{ ucwords($voucher->promo_name) }}</p>
                  ...
                  {{-- <div class="grid">
                    ...
                    <div class="grid gap-1">
                      <p class="ml-2 text-[10px] md:text-[10px] lg:text-[10px] xl:text-[12px]">1. Maksimal {{ $voucher->max_quantity_buyer }} item pembelian</p>
                      <p class="ml-2 text-[10px] md:text-[10px] lg:text-[10px] xl:text-[12px]">2. Minimun transaksi Rp{{ number_format($voucher->min_transaction, 0, ',', '.') }}
                      <p class="ml-2 text-[10px] md:text-[10px] lg:text-[10px] xl:text-[12px]">3. Periode voucher {{ \Carbon\Carbon::parse($voucher->start_date)->translatedFormat('d F Y') }} hingga {{ \Carbon\Carbon::parse($voucher->end_date)->translatedFormat('d F Y') }}</p>
           ...
           <div class="modal fade" id="voucher-{{$voucher->name}}" tabindex="-1" aria-labelledby="voucher-{{$voucher->name}}" aria-hidden="true">
            <div class="modal-dialog modal-sm">
              <div class="modal-content overflow-y-auto" style="max-height:90vh;">
                {{-- <div class="modal-header bg-[#183018]">
                  <h1 class="modal-title text-white text-[12px] lg:text-[16px]">Promo {{$voucher->promo_name}}</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="filter: invert(1);"></button>
                </div> --}}
                <div class="modal-content">
                  <div class="bg-white rounded-lg shadow-md overflow-hidden h-fit">
                    <div class="product-image-container">
                        <img class="card-img-top product-image" src="{{ Storage::url($voucher->image) }}" alt="{{ $voucher->promo_name }}">
                    </div>
                    <div class=" p-2">
                      <div class="grid w-full gap-1">
                        <div class="grid">
                          <p class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px] text-[#183018] font-semibold">{{ ucwords($voucher->promo_name) }}</p>
                          <p class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[12px]">Dapatkan diskon
                            @if ($voucher->discount <= 100)
                              {{ $voucher->discount }}%
                            @else
    ...
    @if (count($brandVouchers) !== 0)
      ...
      <div class="grid-container-promo px-3">
        @foreach ($brandVouchers as $brand)
          <div class="bg-white rounded-lg shadow-md overflow-hidden h-fit">
            <div class="product-image-container">
              <img class="product-image" src="{{ Storage::url($brand->image) }}" alt="{{ $brand->promo_name }}">
            </div>
            <div class=" p-2">
              <div class="grid w-full gap-1">
                <div class="grid">
