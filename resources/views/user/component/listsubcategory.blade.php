            ...
            <div class="border-bottom pb-2">
              ...
              @foreach ($categories as $category)
                  <div class="d-flex align-items-center">
                      <a href="/belanja-{{$category->name}}" class="text-[7px] md:text-[9px] lg:text-[9px] xl:text-[13px]">{{ ucwords(strtolower($category->name)) }}</a>
                      <a class="ml-auto text-[7px] md:text-[9px] lg:text-[9px] xl:text-[13px] text-decoration-none" style="color: #183018;" onclick="toggleListSubCategory(event, '#sub-category-{{$category->name}}', this)">
                        ...
                        @if (count($products) !== 0)
                          <div class="grid-container-shop" style="min-height:48vh;">
                            @foreach ($products as $product)
                              <div onclick="window.location.href = '/{{ $product->product_code }}_product'" class="bg-white rounded-lg shadow-sm  border border-black overflow-hidden h-fit hover:cursor-pointer">
