  ...
  <div class="container-fluid">
    <div class="shadow-sm border border-black rounded-md py-2 py-md-3 my-2 my-md-3">
      <div class="d-flex gap-2 pl-2">
        ...
        <a href="/shop" class="text-black text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Belanja</a>
      </div>
    </div>
  </div>
                  ...
                  @if(isset($subCategories[$category->id]))
                      <div class="grid pb-4 sub-category" id="sub-category-{{$category->name}}" style="display: none;">
                          @foreach ($subCategories[$category->id] as $subcategory)
                              <a href="/belanja-{{$category->name}}-{{ $subcategory->name }}" class="grid text-[7px] md:text-[9px] lg:text-[11px] xl:text-[12px]">{{ $subcategory->name }}</a>
                          @endforeach
                      </div>
                  @endif
              @endforeach
                                            ...
                                            <div class="grid gap-1 text-left p-2">
                                                <div class="flex">
                                                    <div class="flex gap-1">
                                                        ...
                                                        <p class="text-decoration-none text-black text-[8px] md:text-[12px] lg:text-[14px] xl:text-[14px]">@if ($product->rating !== NULL) {{ $product->rating }} @else 0 @endif</p>
                                                    </div>
                                                    <div class="ml-auto">
                                                        @php
                                                ...
                                                <p class="text-decoration-none text-black text-[8px] md:text-[10px] lg:text-[12px] xl:text-[12px]">
                                                    <a href="/{{ $product->product_code }}_product" class="text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $product->product_name }}">{{ Str::limit($product->product_name, 20) }}</a>
                                                </p>
                                                <div class="flex justify-content-start gap-1">
                                                    <p class="text-decoration-none text-black text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Rp {{ number_format($product->regular_price, 0, ',', '.') }}</p>
                                                </div>
                                            </div>
                                            <div class="flex justify-content-between px-2">
                                                @if ($product->stock_quantity == 0)
                                                    <a class="mb-2 py-2 rounded-sm border border-[#183018] shadow-sm w-full bg-danger text-decoration-none text-white p-0 text-[7px] md:text-[12px] lg:text-[10px] xl:text-[12px] flex gap-1 align-items-center justify-content-center hover-red" data-bs-toggle="tooltip" data-bs-placement="top" title="Beritahu Saya Jika Stok Sudah Ada" type="button" style="color:#183018" id="notify-me-{{ $product->id }}" onclick="notifyMe({{ $product->id }})">Stok Habis</a>
                                                @else
                                                    @php
                                                        $inCart = collect($cartItems)->contains('product_id', $product->id);
                                                    @endphp
                                                    @if($inCart)
                                                        <a href="/cart" class="mb-2 py-2 rounded-sm border border-[#183018] shadow-sm w-full bg-[#183018] text-decoration-none text-white p-0 text-[7px] md:text-[10px] lg:text-[10px] xl:text-[10px] flex gap-1 align-items-center justify-content-center hover-red">Cek Keranjang Belanjamu</a>
                                                    @else
                                                        <a href="javascript:void(0);" class="mb-2 py-2 rounded-sm border border-[#183018] hover:border-white shadow-sm w-full hover:bg-[#183018] text-decoration-none text-[#183018] hover:text-white p-0 text-[7px] md:text-[12px] lg:text-[10px] xl:text-[12px] flex gap-1 align-items-center justify-content-center hover-red" onclick="addToCart({{ $product->id }})">+ <i class="fas fa-shopping-cart"></i> Keranjang</a>
                                                    @endif
                                                @endif
                                            </div>
                                        </a>
                                    ...
                                    <div class="bg-white rounded-lg shadow-sm overflow-hidden product-item-except border border-xl">
                                        ...
                                        <div class="grid text-left content-card px-1 py-2 flex-grow-1">
                                            <div class="flex rating-wishlist">
                                                ...
                                                <div class="ml-auto">
                                                    <a title="Tambah ke Favorit" href="javascript:void(0);" class="text-decoration-none {{ $inWishlist ? 'text-[#FF0000]' : 'text-[#183018]' }} p-0 text-[7px] md:text-[12px] lg:text-[10px] xl:text-[12px] grid align-items-center justify-content-between hover-red" onclick="addToWishlist({{ $product->id }})">
                                                        <i class="fas fa-heart text-center"></i>
                                                    </a>
                                                </div>
                                            </div>
      ...
      <div class="modal-body overflow-y-auto custom-scroll" style="max-height:100vh;">
        <form action="" id="form-filter-mobile">
           ...
           <div>
              ...
              <button class="btn text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px] text-white border w-full rounded-md mb-2" type="submit" id="resetFilterMobile"  style="background-color: #183018" onclick="resetFilter()">
                Reset Filter
              </button>
           </div>
        </form>
