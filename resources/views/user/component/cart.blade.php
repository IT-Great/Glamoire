        ...
        <div class="container-fluid">
            <div class="shadow-sm border border-black rounded-sm py-2 py-md-3 my-2 my-md-3">
                <div class="d-flex gap-2 pl-2">
                    ...
                    <a href="#"
                        class="text-black text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Keranjang</a>
                </div>
            </div>
                        ...
                        <div class="container border border-[#183018] rounded shadow-md">
                            <div class="form-check py-2 py-md-3 border-bottom border-[#183018]">
                                <input class="form-check-input" type="checkbox" value="" id="select-all"
                                    onclick="toggleCheckboxes(this)" onchange="toggleSelectAll()">
                                <label
                            ...
                            @foreach ($data as $product)
                                ...
                                @if ($product->product_variant_id !== null)
                                    <div
                                        class="form-check grid border-bottom border-[#183018] py-2 py-md-3 {{ $product->productVariant->variant_stock == 0 ? 'bg-secondary' : '' }}">
                                        <div class="d-flex">
                                            <div class="col-lg-2 col-md-4 col-4 pl-1">
                                                @if ($product->productVariant->variant_stock > 0)
                                                    <input class="form-check-input item-checkbox" type="checkbox"
                                                        value="{{ $product->total }}"
                                                        id="produk_{{ $product->product_variant_id }}" data-type="variant"
                                                        data-price="{{ $product->price }}" onchange="calculateTotal()"
                                                        {{ $product->is_choose == true ? 'checked' : '' }}>
                                                @else
                                                @endif
                                                <img src="{{ Storage::url($product->productVariant->variant_image) }}"
                                                    alt="nama produk"
                                                    class="img-fluid w-100 border border-[#183018] rounded-sm">
                                            </div>
                                            <div class="col-lg-10 col-md-8 col-8 p-0 p-md-2 d-flex flex-column">
                                                @if ($product->productVariant->variant_stock == 0)
                                                    <p
                                                        class="text-danger font-semibold text-[10px] md:text-[12px] lg:text-[12px] xl:text-[14px] hover:cursor-pointer">
                                                        Stok Habis
                                                    </p>
                                                @endif
                                                <p class="hover:cursor-pointer text-[10px] text-black md:text-[12px] lg:text-[12px] xl:text-[14px] {{ $product->productVariant->variant_stock == 0 ? 'text-primary' : '' }}"
                                                    onclick="detailProductVariant('{{ $product->product->product_code }}', '{{ $product->productVariant->sku }}')">
                                                    {{ $product->product->product_name }}</p>
                                                <a
                                                    class="w-fit bg-[#183018] text-white py-1 px-2 rounded-sm text-[10px] md:text-[10px] lg:text-[12px] xl:text-[12px] text-center">
                                                    ...
                                                    <div class="flex ml-auto">
                                                        ...
                                                        @else
                                                            <div class="input-group quantity-detail-produk-variant rounded-sm shadow-sm"
                                                                ...
                                                                <div class="input-group-btn">
                                                                    <button class="btn btn-plus-variant"
                                                                        data-id="{{ $product->product_variant_id }}"
                                                                        data-quantity="{{ $product->productVariant->variant_stock }}"
                                                                        style="height: 32px; width: 32px; display: flex; justify-content: center; align-items: center;"
                                                                        id="plus-btn-product-cart-variant-{{ $product->product_variant_id }}">
                                ...
                                @else
                                    <div
                                        class="form-check grid border-bottom border-[#183018] py-2 py-md-3 {{ $product->product->stock_quantity == 0 ? 'bg-secondary' : '' }}">
                                        <div class="d-flex">
                                            <div class="col-lg-2 col-md-4 col-4 pl-1">
                                                @if ($product->product->stock_quantity > 0)
                                                    <input class="form-check-input item-checkbox" type="checkbox"
                                                        value="{{ $product->total }}"
                                                        id="produk_{{ $product->product_id }}" data-type="product"
                                                        data-price="{{ $product->price }}" onchange="calculateTotal()"
                                                        {{ $product->is_choose == true ? 'checked' : '' }}>
                                                @else
                                                @endif
                                                <img src="{{ Storage::url($product->product->main_image) }}"
                                                    alt="nama produk"
                                                    class="img-fluid w-100 border border-[#183018] rounded-sm">
                                            </div>
                                            <div class="col-lg-10 col-md-8 col-8 p-0 p-md-2 d-flex flex-column">
                                                ...
                                                @endif
                                                <p class="hover:cursor-pointer text-[10px] text-black md:text-[12px] lg:text-[12px] xl:text-[14px] {{ $product->product->stock_quantity == 0 ? 'text-primary' : '' }}"
                                                    onclick="detailProduct('{{ $product->product->product_code }}')">
                                                    {{ $product->product->product_name }}</p>
                                                @php
                                                ...
                                                <div class="flex gap-1">
                                                    @if ($discountedPrice && $discountedPrice < $product->product->regular_price)
                                                        <p
                                                            class="flex justify-content-center text-align-center text-decoration-none text-muted text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px]">
                                                            <del>
                                                                Rp{{ number_format($product->product->regular_price, 0, ',', '.') }}
                                                            </del>
                                                        </p>
                                                        <p
                                                            class="text-decoration-none text-[#183018] text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px]">
                                                ...
                                                <div class="flex mt-auto bottom">
                                                    <div class="flex ml-auto">
                                                        ...
                                                        @else
                                                            <div class="input-group quantity-detail-produk rounded-sm shadow-sm"
                                                                style="width: 120px;">
                                                                <div class="input-group-btn">
                                                                    <button class="btn btn-minus"
                                                                        ...
                                                                        style="height: 32px; width: 32px; display: flex; justify-content: center; align-items: center;"
                                                                        id="minus-btn-product-cart-{{ $product->product_id }}">
                                                                        <i class="fa fa-minus text-xs"></i>
                                                                    </button>
                                                                </div>
                        ...
                        <div class="position-sticky" style="top: 4rem">
                            <div class="mb-3 rounded p-3 bg-white shadow-md border border-[#183018]">
                                <div class="d-flex py-2">
                                    <p class="text-black text-[10px] md:text-[10px] lg:text-[12px] xl:text-[16px]">Total
                                        Harga</p>
                                    <p id="totalPrice"
                                        class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[16px] ml-auto text-black">
                                        {{ 'Rp' . number_format(0, 0, ',', '.') }}</p>
                                </div>
                                <div class="border-top border-[#183018] pt-2">
                                    <button
                                        class="hover:cursor-pointer py-2 text-decoration-none rounded-sm hover:bg-neutral-900 shadow-sm px-3 text-white bg-[#183018] w-full text-[10px] md:text-[10px] lg:text-[12px] xl:text-[16px]"
                                        id="paynow" onclick="checkout()" disabled>
                                        Beli
                                    </button>
                                    </a>
                                </div>
                ...
                @else
                    <div style="min-height:10vh;">
                        <div class="flex align-items-center justify-content-center">
                            <img src="images/cart-empty.png" class="img-fluid"
                                style="width:20%; height:100%; object-fit: cover;" alt="Produk Tidak Ditemukan">
                        </div>
                        <div class="grid align-items-center justify-content-center">
                            <p class="text-danger text-md">Keranjang belanjamu masih kosong nih</p>
                            <button class="btn btn-success rounded-sm w-full" onclick="location.href='/shop'">Mulai
                                Belanja</button>
                        </div>
                    </div>
                @endif
    ...
    <div class="d-lg-none fixed-bottom" style="background-color:#183018;">
        <div class="container-fluid d-flex justify-content-end align-items-center gap-2">
            <div class="grid py-2 text-end">
                ...
                <p id="totalPriceMobile" class="text-[12px] ml-auto text-white">
                    {{ 'Rp' . number_format(0, 0, ',', '.') }}</p>
            </div calss="ml-auto">
            <button class="btn btn-light px-8 w-fit h-fit font-semibold rounded-sm text-[#183018] text-[12px]"
                type="submit" id="paynowmobile" onclick="checkout()" disabled>
                Beli
            </button>
        </div>
    </div>
