...
<style>
    ...
    /* Filter Tabs */
    .order-filter-wrapper {
        background: #FFF; border-radius: 16px; padding: 0; box-shadow: 0 4px 15px rgba(0,0,0,0.02);
        margin-bottom: 1.5rem; overflow-x: auto; white-space: nowrap; border: 1px solid var(--border-color);
    }
    .order-filter-tabs { display: flex; list-style: none; margin: 0; padding: 0; }
    .filter-tab-item {
        padding: 1rem 1.5rem; color: var(--text-muted); font-weight: 600; font-size: 0.9rem;
        text-decoration: none; border-bottom: 2px solid transparent; transition: var(--transition-smooth);
    }
    .filter-tab-item:hover { color: var(--glamoire-dark); }
    .filter-tab-item.active { color: var(--glamoire-dark); border-bottom-color: var(--glamoire-dark); }

    /* Search & Limit Wrapper */
    ...
    .form-select-custom {
        border-radius: 12px; border: 1px solid var(--border-color); font-size: 0.95rem;
        padding: 0.6rem 2rem 0.6rem 1rem; color: var(--text-main); cursor: pointer;
        box-shadow: 0 4px 15px rgba(0,0,0,0.02); outline: none;
    }
    .form-select-custom:focus { border-color: var(--glamoire-dark); box-shadow: 0 0 0 3px rgba(24,48,24,0.1); }

    /* Custom Pagination */
    .custom-pagination { display: flex; list-style: none; padding: 0; margin: 0; gap: 5px; flex-wrap: wrap; }
    .custom-pagination .page-item .page-link {
        border: 1px solid var(--border-color); background: #FFF; color: var(--text-main);
        padding: 0.5rem 0.85rem; border-radius: 8px; font-size: 0.9rem; font-weight: 500;
        text-decoration: none; transition: var(--transition-smooth); display: block;
    }
    .custom-pagination .page-item.active .page-link {
        background: var(--glamoire-dark); color: #FFF; border-color: var(--glamoire-dark);
    }
    .custom-pagination .page-item:not(.active):not(.disabled) .page-link:hover {
        background: var(--glamoire-light); border-color: var(--glamoire-dark);
    }
    .custom-pagination .page-item.disabled .page-link {
        color: var(--text-muted); background: #F3F4F6; cursor: not-allowed; border-color: var(--border-color);
    }
    ...
    .order-item-row { display: flex; flex-wrap: wrap; gap: 1.5rem; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px dashed var(--border-color); align-items: center; }
    .order-item-row:last-child { margin-bottom: 0; padding-bottom: 0; border-bottom: none; }
    .order-item-img { width: 90px; height: 90px; border-radius: 8px; object-fit: cover; border: 1px solid var(--border-color); cursor: pointer; transition: transform 0.3s; }
    .order-item-img:hover { transform: scale(1.05); }
    .order-item-info { flex: 1; min-width: 200px; cursor: pointer;}
    .order-item-brand { font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; font-weight: 700; }
    .order-item-name { font-size: 1.05rem; font-weight: 600; color: var(--text-main); margin-bottom: 0.2rem; }
    .order-item-variant { font-size: 0.85rem; color: var(--text-muted); background: var(--glamoire-light); padding: 2px 8px; border-radius: 4px; display: inline-block; margin-bottom: 0.5rem; }
    .order-item-qty { font-size: 0.9rem; color: var(--text-main); font-weight: 500; }
    .order-item-price { text-align: right; min-width: 120px; }
    .order-item-price span { display: block; font-size: 0.8rem; color: var(--text-muted); margin-bottom: 4px; }
    .order-item-price strong { font-size: 1.15rem; color: var(--text-main); }
    ...
    .btn-glamoire { background: var(--glamoire-dark); color: #FFF; border: none; padding: 0.75rem 2rem; border-radius: 50px; font-weight: 600; font-size: 0.95rem; text-decoration: none;}
    .btn-outline-glamoire { background: transparent; color: var(--glamoire-dark); border: 1px solid var(--glamoire-dark); padding: 0.6rem 1.5rem; border-radius: 50px; font-weight: 600; font-size: 0.85rem; text-decoration: none;}
</style>
...
<div class="md:px-20 lg:px-24 xl:px-24 2xl:px-48 pt-4 pb-5">
    <div class="container-fluid">
        ...
        <h2 class="mb-4" style="font-family: 'Poppins', sans-serif; font-size: 1.8rem; font-weight: 700; color: var(--text-main);">Riwayat Pesanan</h2>

        <!-- FORM CONTROLS: SEARCH & PER PAGE -->
        <form action="{{ route('user.orders') }}" method="GET" id="filterForm" class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <input type="hidden" name="status" value="{{ request('status') }}">
            ...
            <!-- PER PAGE DROPDOWN (KANAN) -->
            <div class="d-flex align-items-center gap-2">
                <span class="text-muted fw-medium" style="font-size: 0.9rem;">Tampilkan:</span>
                <select name="per_page" class="form-select-custom" onchange="document.getElementById('filterForm').submit()">
        ...
        <!-- FILTER TABS (Menjaga relasi Search dan Per_page) -->
        <div class="order-filter-wrapper">
            <ul class="order-filter-tabs">
                @php $params = ['search' => request('search'), 'per_page' => request('per_page')]; @endphp
                <li><a href="{{ route('user.orders', array_merge(['status' => ''], $params)) }}" class="filter-tab-item {{ request('status') == '' ? 'active' : '' }}">Semua</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'pending'], $params)) }}" class="filter-tab-item {{ request('status') == 'pending' ? 'active' : '' }}">Belum Dibayar</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'processing'], $params)) }}" class="filter-tab-item {{ request('status') == 'processing' ? 'active' : '' }}">Diproses</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'delivery'], $params)) }}" class="filter-tab-item {{ request('status') == 'delivery' ? 'active' : '' }}">Dikirim</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'completed'], $params)) }}" class="filter-tab-item {{ request('status') == 'completed' ? 'active' : '' }}">Selesai</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'cancelled'], $params)) }}" class="filter-tab-item {{ request('status') == 'cancelled' ? 'active' : '' }}">Dibatalkan</a></li>
                <li><a href="{{ route('user.orders', array_merge(['status' => 'returned'], $params)) }}" class="filter-tab-item {{ request('status') == 'returned' ? 'active' : '' }}">Retur/Gagal</a></li>
            </ul>
        </div>
        ...
        @if ($orders->count() > 0)
            @foreach ($orders as $order)
                <div class="order-card">
                    <!-- ... HEADER ORDER (SAMA SEPERTI SEBELUMNYA) ... -->
                    <div class="order-header">
                        <div class="order-meta">
                            <div class="order-meta-item d-none d-md-block">
                                <span>Tanggal Pesanan</span>
                        ...
                        <div>
                            @php
                                ...
                                $statusClass = '';
                                if ($order->return_status !== null) {
                                    if ($order->return_status == 'requested') { $statusText = 'Menunggu Validasi Retur'; $statusClass = 'bg-warning text-dark'; }
                                    elseif ($order->return_status == 'approved') { $statusText = 'Retur Disetujui'; $statusClass = 'bg-success text-white'; }
                                    elseif ($order->return_status == 'rejected') { $statusText = 'Retur Ditolak'; $statusClass = 'bg-danger text-white'; }
                                } else {
                                    switch ($order->status) {
                                        case 'completed': $statusText = 'Selesai'; $statusClass = 'status-completed'; break;
                                        case 'pending': $statusText = 'Menunggu Pembayaran'; $statusClass = 'status-pending'; break;
                    ...
                    <!-- BODY ORDER (SAMA SEPERTI SEBELUMNYA) -->
                    <div class="order-body">
                        @foreach ($order->items as $item)
                            @if ($item->product)
                                <div class="order-item-row" onclick="{{ $item->product_variant_id ? "detailProductVariant('{$item->product->product_code}', '{$item->productVariant->sku}')" : "detailProduct('{$item->product->product_code}')" }}">
                    ...
                    <!-- FOOTER ORDER (SAMA SEPERTI SEBELUMNYA) -->
                    <div class="order-footer">
                        <div class="order-total-box">
                            <span>Total Belanja:</span>
                            <strong>Rp{{ number_format($order->total_amount, 0, ',', '.') }}</strong>
            ...
            <!-- ========================================== -->
            <!-- CUSTOM PAGINATION (DENGAN LOGIKA ELLIPSIS) -->
            <!-- ========================================== -->
            <div class="d-flex justify-content-between align-items-center flex-wrap mt-5 gap-3 border-top pt-4">

                <!-- "Showing x to y from z items" -->
                <div class="text-muted" style="font-size: 0.9rem;">
                    Showing <strong>{{ $orders->firstItem() ?? 0 }}</strong> to <strong>{{ $orders->lastItem() ?? 0 }}</strong> from <strong>{{ $orders->total() }}</strong> items
                </div>
                ...
                @if ($orders->hasPages())
                    <ul class="custom-pagination">
                        ...
                        @else
                            <li class="page-item"><a class="page-link" href="{{ $orders->appends(request()->query())->previousPageUrl() }}"><i class="fas fa-chevron-left"></i></a></li>
                        @endif
                        ...
                        @if ($last <= 7)
                            ...
                            @endfor
                        @else
                            {{-- Skenario A: User berada di halaman awal (1 sampai 4) --}}
                            @if ($current <= 4)
                                @for ($i = 1; $i <= 5; $i++)
                                    <li class="page-item {{ $i == $current ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $orders->appends(request()->query())->url($i) }}">{{ $i }}</a>
                                ...
                                <li class="page-item"><a class="page-link" href="{{ $orders->appends(request()->query())->url($last) }}">{{ $last }}</a></li>

                            {{-- Skenario C: User berada di halaman akhir (4 halaman terakhir) --}}
                            ...
                            {{-- Skenario B: User berada di tengah-tengah --}}
                            @else
                                <li class="page-item"><a class="page-link" href="{{ $orders->appends(request()->query())->url(1) }}">1</a></li>
                                <li class="page-item disabled"><span class="page-link">...</span></li>
                        ...
                        @if ($orders->hasMorePages())
                            <li class="page-item"><a class="page-link" href="{{ $orders->appends(request()->query())->nextPageUrl() }}"><i class="fas fa-chevron-right"></i></a></li>
                        @else
        ...
        @else
            <div class="order-card p-5 text-center border-0" style="background: transparent;">
                <img src="{{ asset('images/cart-empty.png') }}" alt="Kosong" style="width: 150px; opacity: 0.7; margin-bottom: 1rem;">
                <h4 style="font-weight: 700; color: var(--text-main);">Pesanan Tidak Ditemukan</h4>
                <p class="text-muted">Mungkin Anda belum melakukan pesanan atau coba gunakan kata kunci lain.</p>
                <a href="/shop" class="btn-glamoire mt-3 text-decoration-none">Mulai Belanja</a>
            </div>
        @endif
