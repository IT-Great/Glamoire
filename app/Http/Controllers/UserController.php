...
class UserController extends Controller
{
    ...
    public function account()
    {
        try {
                    ...
                    if ($order->payment && $order->payment->status !== 'completed' && $order->payment->status !== 'failed' && $order->payment->status !== 'cancelled') {
                        ...
                        if (isset($payment_status['transaction_status'])) {
                            ...
                            if ($payment_status['transaction_status'] == 'SETLD') {
                                ...
                                // Update status voucher jika digunakan
                                $useVoucherNewUser = VoucherNewUser::where('user_id', $id)
                                    ->where('code', $order->voucher_promo)
                                    ->first();
                                ...
                                $voucherUsed = ($useVoucherNewUser == NULL && $order->voucher_promo) ? Promo::where('promo_code', $order->voucher_promo)->first() : NULL;
                                $ongkirUsed = $order->voucher_ongkir !== null ? Promo::where('promo_code', $order->voucher_ongkir)->first() : null;
                                ...
                                foreach ($orderItems as $product) {
                                    if ($product->product_variant_id !== null) {
                                        $productVariant = ProductVariations::find($product->product_variant_id);
                                        $productMain = Product::find($productVariant->product_id ?? $product->product_id);
                                        ...
                                        $productMain = Product::find($product->product_id);

                                        if ($productMain) {
                                            $productMain->stock_quantity -= $product->quantity;
                                            $productMain->sale += $product->quantity;
                                            $productMain->save();
                                        }
            ...
            Log::error('Error at account dashboard: ' . $err->getMessage());
    ...
    public function removeFromWishlist(Request $request)
    {
        try {
            ...
            Log::error('Error remove wishlist: ' . $th->getMessage());
        }
    }
    ...
    {
        try {
            Shipping_address::where('id', $request->input('address_id'))->delete();
            return response()->json(['success' => true, 'message' => 'Berhasil Menghapus Alamat Pengiriman']);
        } catch (Exception $err) {
            dd($err);
        }
