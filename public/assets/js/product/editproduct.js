...
//         // If no variants, add a default variant type
//         addNewVariantType();
//         return;
//     }
...
//                     }>Flavor</option>
//                     <option value="ukuran" ${
//                         type === "ukuran" ? "selected" : ""
//                     }>Size</option>
//                     <option value="desain" ${
//                         type === "desain" ? "selected" : ""
//                     }>Desain</option>
//                     <option value="durabilitas" ${
//                         type === "durabilitas" ? "selected" : ""
//                     }>Durabilitas</option>
//                     <option value="fungsionalitas" ${
//                         type === "fungsionalitas" ? "selected" : ""
//                     }>Fungsionalitas</option>
//                     <option value="tekstur" ${
//                         type === "tekstur" ? "selected" : ""
//                     }>Tekstur</option>
//                     <option value="bahan" ${
//                         type === "bahan" ? "selected" : ""
//                     }>Bahan</option>
...
//                 </select>
//             </div>
//             <small class="text-muted">Pilih nilai varian atau tambahkan yang baru jika Anda tidak menemukan opsi yang sesuai.</small>
//         `;

//             variantContainer.appendChild(variantTypeDiv);

//             // Initialize Select2 for the new elements with tags enabled
//             const variantTypeSelect = variantTypeDiv.querySelector(
//                 ".select2-variant-type"
//             );
//             const variantValuesSelect = variantTypeDiv.querySelector(
...
//                 </div>
//                 <div class="variant-images mt-2" style="display: ${
//                     variant.image ? "block" : "none"
//                 }">
...
//                            name="variant_expired[${index}]"
//                            placeholder="Masukan Expired Produk"
//                            value="${variant.variant_expired || ""}"> <!-- Use variant_expired here -->
//                 </div>
//             </td>
...
//     // Initialize datepickers for the new rows
...
//             const existingImagePreview = this.closest(
//                 ".variant-images"
//             ).querySelector(".existing-image-preview");
...
//                 this.closest("td").querySelector(".variant-images");
...
//         } else {
//             // No existing variants, add a default variant type
//             addNewVariantType();
//         }
//     }
...
//                             <input type="text"
//                                    class="form-control datepicker2"
//                                    name="new_variant_expired[${typeIndex}][${valueIndex}]"
//                                    placeholder="Masukan Expired Produk">
//                         </div>
//                     </td>
//                     `;
...
//                 <div class="variant-values">
//                 <label class="form-label mt-4">Variant Values</label>
//                     <select class="select2 form-select multiple-remove" name="variant_values[${
//                         variantTypes - 1
...
//             // Initialize Select2 for the new variant type
//             const newVariantTypeSelect = newVariantType.querySelector(
//                 ".select2-variant-type"
//             );
//             const newVariantValuesSelect = newVariantType.querySelector(
//                 ".variant-values select"
//             );

//             // Initialize variant type select
...
//             // Initialize variant values select
//             initializeSelect2(newVariantValuesSelect, {
//                 tags: true,
//                 tokenSeparators: [",", " "],
//             });

//             // Update variant values when type changes
//             newVariantTypeSelect.addEventListener("change", function () {
//                 updateVariantValues(this);
//             });

//             // Initial update of variant values
//             updateVariantValues(newVariantTypeSelect);

//             // Update variant table
//             updateVariantTable();

//             // Disable add variant button if max reached
//             if (variantTypes >= 2) {
//                 addVariantTypeBtn.disabled = true;
//                 addVariantTypeBtn.classList.add("disabled");
//             }
...
//                     const fileInput =
//                         imageUploadArea.find('input[type="file"]');
//                     fileInput.val("");
...
//     updateVariantValues(
//         document.querySelector('select[name="variant_type[]"]')
//     );
//     updateVariantTable();
...
//             const existingNewImage = singleUploadContent.querySelector(
//                 ".new-image-container"
//             );
//             if (existingNewImage) {
...
//                 "image-preview-container new-image-container";
...
//             singleUploadContent.insertBefore(
//                 newImageContainer,
//                 singleUploadContent.firstChild
//             );
//         };
...
//     document
//         .querySelector(".file-upload-content")
...
//     const existingNewLabel =
//         fileUploadContent.querySelector(".new-images-label");
//     if (!existingNewLabel) {
...
//     let newImagesContainer = fileUploadContent.querySelector(
...
//                     const newImagesLabel =
//                         fileUploadContent.querySelector(".new-images-label");
//                     if (newImagesLabel) newImagesLabel.remove();
//                     const newImagesContainer = fileUploadContent.querySelector(
//                         ".new-images-container"
//                     );
...
//             const existingVideoContent = document.getElementById(
//                 "existing-video-content"
//             );
//             if (existingVideoContent) {
//                 existingVideoContent.style.display = "none";
//             }
...
//     // Hapus video baru dan kembalikan tampilan upload
...
//     );
//     if (existingVideoContent) {
//         existingVideoContent.style.display = "block";
//     }
// }

// function removeExistingVideo() {
//     // Menandai bahwa video lama harus dihapus
//     const existingVideoContent = document.getElementById(
//         "existing-video-content"
//     );
//     if (existingVideoContent) {
...
// // HANDLE FORMAT DATE PICKER product variant
...
// // HANDLE FORMAT DATE PICKER product utama
// document.addEventListener("DOMContentLoaded", function () {
...
//     if(stockQuantityInput) stockQuantityInput.addEventListener("input", updateVariantTable);
//     if(regularPriceInput) regularPriceInput.addEventListener("input", updateVariantTable);
//     if(weightProductInput) weightProductInput.addEventListener("input", updateVariantTable);
...
//                     </td>
//                     <td class="text-center align-middle">
//                         <button type="button" class="btn btn-sm btn-danger btn-delete-variant" data-id="" title="Batal Tambah Varian">
...
//     // Expose updateVariantTable globally if needed by other functions
//     window.updateVariantTable = updateVariantTable;
...
//     const firstVariantTypeSelect = document.querySelector('select[name="variant_type[]"]');
//     if(firstVariantTypeSelect) {
//         // Initialize the first variant type
//         updateVariantValues(firstVariantTypeSelect);
//         if(typeof window.updateVariantTable === 'function') window.updateVariantTable();
//     }
...
//     galleryContainer.appendChild(oldImagesContainer);
//     const fileUploadContentElement = document.querySelector(".file-upload-content");
//     if(fileUploadContentElement) {
//         fileUploadContentElement.appendChild(galleryContainer);
//     }
// }
...
//         if (selectedFiles.length === 0 && oldImages === 0 && imageError) {
//             event.preventDefault();
//             imageError.textContent = "Please upload at least one image.";
//             imageError.style.display = "block";
//             imageError.scrollIntoView({ behavior: "smooth", block: "center" });
...
// ==========================================
// 2. FUNGSI INISIALISASI (UTILITIES)
// ==========================================
function initializeSelect2WithAddOption(selectElement) {
    $(selectElement).select2({ tags: true, tokenSeparators: [",", " "], width: "100%" });
}

function initializeSelect2(selectElement, options = {}) {
    $(selectElement).select2({ tags: true, tokenSeparators: [",", " "], width: "100%", closeOnSelect: false, ...options });
}

function initializeDatePickers() {
    flatpickr(".datepicker2", { enableTime: false, dateFormat: "Y-m-d", time_24hr: true, minuteIncrement: 1 });
}
...
function addNewVariantType() {
    ...
    if (variantTypes < 2) {
        ...
        initializeSelect2(newVariantTypeSelect, {
            tags: true,
            createTag: function (params) { return { id: params.term, text: params.term, newOption: true }; },
            closeOnSelect: true,
        });

        initializeSelect2(newVariantValuesSelect, { tags: true, tokenSeparators: [",", " "] });
        updateVariantValues(newVariantTypeSelect);
...
function updateVariantTableWithExistingData(variants) {
    ...
    variants.forEach((variant, index) => {
        ...
        row.innerHTML = `
            <td>
                <div class="form-check form-switch">
                    <input class="form-check-input use-variant-image" type="checkbox" id="useVariantImage${index}" name="use_variant_image[${index}]" value="1" ${variant.image ? "checked" : ""}>
                    <label class="form-check-label" for="useVariantImage${index}">Use image</label>
                </div>
                <div class="variant-images mt-2" style="display: ${variant.image ? "block" : "none"}">
                    <div class="existing-image-preview mb-2">
            ...
            <td class="align-middle">${variant.type}: ${variant.value}</td>
            <td class="align-middle"><input type="number" class="form-control" name="variant_price[${index}]" placeholder="Price" min="0" step="0.01" value="${variant.price}"></td>
            <td class="align-middle"><input type="number" class="form-control" name="variant_stock[${index}]" placeholder="Stock" min="0" value="${variant.stock}"></td>
            <td class="align-middle"><input type="number" class="form-control" name="variant_weight[${index}]" placeholder="Weight" min="0" value="${variant.weight}"></td>
            <td class="align-middle">
                <div class="input-group">
                    <span class="input-group-text"><i class="bi bi-calendar"></i></span>
                    <input type="text" class="form-control datepicker2" name="variant_expired[${index}]" placeholder="Masukan Expired" value="${variant.variant_expired || ""}">
    ...
    Object.entries(variantsByType).forEach(([type, typeVariants], typeIndex) => {
        ...
        <div class="d-flex align-items-center">
            <select class="select2-variant-type form-select me-2" name="variant_type[]">
                ...
                <option value="tekstur" ${type === "tekstur" ? "selected" : ""}>Tekstur</option>
                <option value="bahan" ${type === "bahan" ? "selected" : ""}>Bahan</option>
                ${!["warna", "aroma", "rasa", "ukuran", "desain", "durabilitas", "fungsionalitas", "tekstur", "bahan"].includes(type) ? `<option value="${type}" selected>${type}</option>` : ""}
            </select>
        </div>
        <small class="text-muted">Pilih jenis varian atau tambahkan yang baru.</small>
        ...
        initializeSelect2(variantValuesSelect, { tags: true, tokenSeparators: [",", " "] });
    });
...
function readURLVideo(input) {
    if (input.files && input.files[0]) {
        ...
        const reader = new FileReader();
        reader.onload = function (e) {
            const existingVideoContent = document.getElementById("existing-video-content");
            if (existingVideoContent) existingVideoContent.style.display = "none";
...
function removeVideo(element) {
    ...
    const existingVideoContent = document.getElementById("existing-video-content");
    if (existingVideoContent) existingVideoContent.style.display = "block";
}
    ...
    Array.from(files).forEach((file) => {
        ...
        reader.onload = function (e) {
            ...
            imgClose.onclick = function () {
                ...
                if (selectedFiles.length === 0) {
                    const lbl = fileUploadContent.querySelector(".new-images-label");
                    if (lbl) lbl.remove();
                    const cnt = fileUploadContent.querySelector(".new-images-container");
                    if (cnt) cnt.remove();
                }
            };
...
// ==========================================
// 5. DOM CONTENT LOADED (MAIN EXECUTION)
// ==========================================
document.addEventListener("DOMContentLoaded", function () {
    // A. Form Submission Multiple Images Handler
    const formElement = document.querySelector("form");
    if(formElement) {
        formElement.addEventListener("submit", function (event) {
            const imageError = document.getElementById("image-error");
    ...
    // B. Main Datepicker Init
    const dateExpiredInput = document.getElementById("date_expired");
    if(dateExpiredInput) {
        flatpickr("#date_expired", { enableTime: false, dateFormat: "Y-m-d", time_24hr: true, minuteIncrement: 1 });
    }

    // C. Variant Logic Setup
    const variantTableBody = document.getElementById("variant-table-body");
    const stockQuantityInput = document.querySelector('input[name="stock_quantity"]');
    const regularPriceInput = document.querySelector('input[name="regular_price"]');
    const weightProductInput = document.querySelector('input[name="weight_product"]');
    ...
    // Initialize existing variants using injected object from blade
    if (typeof productData !== "undefined" && productData.variants) {
        if (productData.variants.length > 0) {
            initializeExistingVariants(productData.variants);
        } else {
